...
(function initWindowManager() {
  ...
  windows.forEach(win => {
    ...
    titlebar.addEventListener('mousedown', (e) => {
      ...
      // Override default CSS positions
  ...
  document.querySelectorAll('.os-btn').forEach(btn => {
    btn.addEventListener('click', (e) => {
      ...
      if (btn.classList.contains('os-close') || btn.classList.contains('os-min')) {
        win.classList.add('hidden');
        win.classList.remove('active');
        updateTaskbar();
      }
      
      if (btn.classList.contains('os-max')) {
        if (win.style.width === '100vw') {
          // Restore
          win.style.width = win.dataset.origWidth || '480px';
          win.style.height = win.dataset.origHeight || '85vh';
          win.style.left = win.dataset.origLeft || '10vw';
          win.style.top = win.dataset.origTop || '10vh';
        } else {
          // Maximize
  ...
  // Handle old links to steam/color tools
  const oldLinks = [
    { trigger: 'go-steam', target: 'win-steam' },
    { trigger: 'go-color', target: 'win-color' }
  ];
  oldLinks.forEach(link => {
    const el = document.getElementById(link.trigger);
    if (el) {
      // Overwrite previous click handlers cleanly
      const newEl = el.cloneNode(true);
      el.parentNode.replaceChild(newEl, el);
      newEl.addEventListener('click', (e) => {
        e.preventDefault();
        const win = document.getElementById(link.target);
        if (win) {
          win.classList.remove('hidden');
          topZIndex++;
          win.style.zIndex = topZIndex;
