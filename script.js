...
(function initGardenPage() {
  ...
  // ---- Khỉ "thật" thỉnh thoảng đu ngang qua Garden ----
  // Hoàn toàn độc lập với monkeyState/spawnAndFetchBanana của khỉ gốc:
  // đây chỉ là một bản clone DÙNG ĐỂ TRÌNH DIỄN, tự sinh ra rồi tự xóa
  // sau mỗi lượt đu, không có id, không đụng vào DOM/logic bắt chuối thật.
  let swingTimer = null;
  let activeSwinger = null;

  function spawnSwingingMonkey() {
    if (!gardenOpened || !gardenScene) return;

    const monkeySource = document.getElementById('css-monkey');
    if (!monkeySource) return;

    const swinger = monkeySource.cloneNode(true);
    swinger.removeAttribute('id');
    swinger.classList.remove('idle', 'eating');
    swinger.classList.add('swinging', 'garden-monkey-swinger');
    // Bỏ thought-bubble và banana cầm tay vì đây chỉ là khỉ "lướt qua"
    swinger.querySelector('.monkey-thought')?.remove();
    swinger.querySelector('.monkey-held-banana')?.remove();

    gardenScene.appendChild(swinger);
    activeSwinger = swinger;

    // Trigger animation ở frame kế tiếp để đảm bảo CSS transition áp dụng đúng
    requestAnimationFrame(() => {
      swinger.classList.add('swing-active');
    });

    // Animation dài 4.2s (khớp với @keyframes gardenMonkeySwingAcross) — dọn dẹp sau đó
    setTimeout(() => {
      if (swinger.parentElement) swinger.remove();
      if (activeSwinger === swinger) activeSwinger = null;
    }, 4300);
  }
  ...
  function scheduleNextSwing() {
    ...
    // Khỉ đu qua ngẫu nhiên mỗi 15–25 giây, đủ thưa để bất ngờ, không gây rối mắt
  ...
  function stopSwinging() {
    clearTimeout(swingTimer);
    swingTimer = null;
    if (activeSwinger && activeSwinger.parentElement) {
      activeSwinger.remove();
    }
    activeSwinger = null;
  }
  ...
  function setAmbience(on) {
    ...
    if (on) {
      ...
      // Đêm khuya rồi, khỉ cũng nghỉ đu dây cho hợp không khí yên tĩnh
      stopSwinging();
    } else {
      clearInterval(fireflyInterval);
      ...
      // Trời sáng lại, khỉ tiếp tục đu dây như cũ
      if (gardenOpened) scheduleNextSwing();
    }
  }
