...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (bgMuteBtn) {
    bgMuteBtn.addEventListener('click', () => {
      if (!bgYtPlayer || typeof bgYtPlayer.isMuted !== 'function') return;
      isBgMuted = bgYtPlayer.isMuted();
      if (isBgMuted) {
        bgYtPlayer.unMute();
        bgYtPlayer.setVolume(50);
        bgMuteBtn.innerHTML = '🎵';
        bgMuteBtn.style.color = '#35e8ff';
        bgMuteBtn.style.textShadow = '0 0 8px #35e8ff';
      } else {
        bgYtPlayer.mute();
        bgMuteBtn.innerHTML = '🔕';
        bgMuteBtn.style.color = '';
        bgMuteBtn.style.textShadow = 'none';
      }
    });
  }
});
